...
if st.session_state.dark:
    st.markdown("<style>body{background:#1a1a2e;color:#eee}.stApp{background:#1a1a2e}</style>", unsafe_allow_html=True)
...
for k_no in kosular:
    ...
    with st.expander(f"🏁 {k_no}. Koşu | {row['Saat']} | {row['Mesafe']}m {pist_goster} | {cins_goster}"):
        pist_str = str(row["Pist"]) if not pd.isna(row["Pist"]) else ""
        if "Cim" in pist_str or "Çim" in pist_str:
            st.caption("🌧️ Çim pist, yağmurda ağırlaşır.")
        elif "Kum" in pist_str:
            st.caption("🏜️ Kum pist, stabil.")
        ...
        if len(puanlar) >= 3:
            ...
            if fark > 15:
                st.info(f"🎯 **Tek geç:** {puanlar[0]['At']}")
            elif fark > 8:
                st.info(f"🔒 **İkili:** {puanlar[0]['At']} - {puanlar[1]['At']}")
            else:
                st.info(f"🌐 **Geniş:** {puanlar[0]['At']}, {puanlar[1]['At']}, {puanlar[2]['At']}")
        ...
        for i, c in enumerate(["#FFD700", "#C0C0C0", "#CD7F32"]):
            if i < len(puanlar):
                s = puanlar[i]
                with cols[i]:
                    st.markdown(f"<div style='background:{c};padding:8px;border-radius:8px;'>{['🥇','🥈','🥉'][i]} #{s['Sıra']} {s['At'][:20]}<br>Puan: {s['Puan']}</div>", unsafe_allow_html=True)
