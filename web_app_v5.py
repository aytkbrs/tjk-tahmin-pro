...
st.caption("Ganyan %45.9 | Plase %77.7 | Karanlık mod | Akıllı kupon")

if df.empty:
    st.warning("Yarış verisi yok. Lütfen güncelle butonuna bas.")
    if st.button("🔄 Güncelle"): subprocess.run(["python","tjk_cekici.py"], capture_output=True); st.rerun()
    st.stop()

if st.sidebar.button("🔄 Yarışları Güncelle"):
    subprocess.run(["python","tjk_cekici.py"], capture_output=True)
    st.rerun()
